...
function renderTabs() {
  ...
  // ⛔ 08-27: заплатка снята. 08-26 спираль была цельной картинкой из банка, слог
  // на ней печатать было негде, и единицу прятали от слоговой ступени. В тот же
  // вечер линию стал чертить код, а в центр встал слог — материал научился, а
  // запрет остался и прожил сутки (закон 17: перезаказал материю — пересмотри
  // всё, что про неё записано). Поймал автор: «нельзя выбрать 1 на лист».
...
function renderRows() {
  ...
  // Единица есть на ОБЕИХ ступенях: слог печатается в центре спирали ровно так
  // же, как у финиша полосы (`propisi.render_propisi`, ветка `spiral-syll`).
  const nums = [1, 2, 3];
  nums.forEach((n) => {
...
function fitFrame() {
  ...
  // ⚠ 08-26, второй заход. Резерв был заведён утром и НЕ РАБОТАЛ: подписей под
  // листом две — скрытая «Вышла новая версия» и видимая «Как это устроено», —
  // а `querySelector` берёт ПЕРВУЮ, то есть скрытую, высотой ноль. Лист забирал
  // высоту целиком, и подпись уезжала ровно на одну строку (слово автора: «на
  // маке кнопка скрыта, буквально одну полосу нужно прокрутить»).
  // Считаем ВСЕ видимые подписи: когда появится «новая версия», место под неё
  // тоже зарезервируется само.
  const feet = [...document.querySelectorAll('.paper-foot')]
    .filter((e) => !e.hidden && e.offsetParent !== null);
  const footH = feet.reduce(
    (sum, e) => sum + Math.ceil(e.getBoundingClientRect().height), 0)
    + (feet.length ? 4 : 0);
...
function renderMoat(st) {
  ...
  // из НАЗВАНИЙ строк — они и есть объяснение. Показываем, а не рассказываем.
  ...
  if (starsBox) {
    for (let n = 1; n <= 5; n += 1) {
      ...
      b.onclick = () => {
        ...
        // ⚠ 08-26: «Если есть время» снято и отсюда — та же оговорка, что в
        // поле ниже, и она стояла дважды на одном экране.
        $('ask-q').textContent = 'Спасибо. Что поправить?';
...
function renderMethod() {
  ...
  // ⚠ 08-26, слово автора: «начни с той информации, которая актуальна для этого
  // КОНКРЕТНОГО звука». Числа рва стоят выше и уже про этот звук; здесь второе
  // такое же — чего у звука НЕ БЫВАЕТ и почему. Это ровно тот случай, когда
  // показать нечего: позиции нет, значит на экране её нет, — и молчать об этом
  // нельзя (закон 12). Причина берётся у движка (`positions_by_sound.why`),
  // своей формулировки здесь не заводим.
  {
    const pos = ((S.cfg && S.cfg.positions_by_sound) || {})[S.sound] || [];
    const gone = pos.filter((p) => !p.ok && p.why);
    if (gone.length) {
      const sec = el('section', 'method-sec');
      sec.appendChild(el('h3', null,
        'Чего у звука [' + (S.cfg.sounds.find((x) => x.key === S.sound) || {}).label + '] не бывает'));
      gone.forEach((p) => {
        const d = el('div', 'method-rule');
        const t2 = el('div', 'txt');
        // Метку позиции НЕ приписываем: причина от движка сама её называет
        // («звонкий согласный В КОНЦЕ СЛОВА оглушается»), и склейка давала
        // «В конце слова — Звонкий согласный в конце слова…».
        t2.append(p.why);
        d.appendChild(t2);
        sec.appendChild(d);
      });
