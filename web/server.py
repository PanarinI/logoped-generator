...
class Handler(BaseHTTPRequestHandler):
    ...
    CACHE_LIVE = "no-store"
    CACHE_CODE = "no-cache"
    # ⚠ 08-26, поймал автор: «в проде явно что-то не так» — на листе стояли
    # СТАРЫЕ машины, хотя сервер отдавал новые файлы (сверил md5 прода и диска —
    # совпали). Причина здесь: при `max-age=86400` браузер сутки НЕ СПРАШИВАЕТ
    # сервер вовсе. ETag рядом лежал и не работал: пока ответ «свежий», его не
    # смотрят.
    # Цена ошибки не в авторе, а в логопедах: заменили банк — и у каждого, кто
    # открывал лист сегодня, картинки останутся вчерашними ещё сутки. За один
    # день 08-26 банки переменились трижды.
    # Теперь картинки РЕВАЛИДИРУЮТСЯ: браузер спрашивает и почти всегда
    # получает 304 без тела — это дешевле мегабайта и честнее суток лжи.
