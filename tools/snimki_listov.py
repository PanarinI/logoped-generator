...
def embed_images(base: str, html: str, cache: Dict[str, bytes]) -> Tuple[str, int]:
    ...
    # ⚠ 2026-08-26: к списку добавлены `/dorozhka/` и `/metki/`. Банк звуковой
    # дорожки (герои, цели, фоны) и метки слоговой появились 08-26, а список
    # путей остался прежним — снимок дорожки вышел бы с битыми квадратами
    # ровно так же, как это уже случалось со спрайтами сцены 08-24.
    # ⚠ Адрес несёт штамп версии `?v=…` (см. `_stamp_pictures`), поэтому в
    # шаблоне после пути стоит `[^"]+`, а не имя файла: хвост тоже часть адреса,
    # и без него запрос к серверу уходит не туда.
