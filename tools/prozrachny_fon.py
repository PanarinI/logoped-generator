...
MAX_SHARE = 0.985    # почти всё — уже не рисунок; грубая страховка

# ⚠ 08-26, две неверные защиты подряд — записываю обе, чтобы не завести третью.
# 1) ДОЛЯ СНЯТОГО (0.88) — плохая мера: у редких рисунков (фоны, пунктирные
#    спирали, трава, камыш) бумаги честно больше 88 %, и двадцать файлов
#    остались непрозрачными без вины.
# 2) «ЗАЛИВКА ВОШЛА ВНУТРЬ» — я думал спросить у снятого пикселя, есть ли
#    чернила со всех четырёх сторон. Не работает: у картинки с НЕСКОЛЬКИМИ
#    предметами бумага МЕЖДУ ними тоже окружена чернилами — солнце слева, туча
#    справа, холм снизу. Фон неба был обвинён зря.
# Настоящая защита здесь только одна — глаза (локальный закон 6). Цена ошибки
# при этом мала: заливка НИКОГДА не трогает чернила (по тёмному она не ходит),
# и худшее, что бывает при разомкнутом контуре, — прозрачная дырка внутри
# предмета. На белой бумаге она неотличима от белого; видно её только поверх
# фона. Поэтому машинная страховка оставлена грубой, а результат смотрится.
...
def strip_paper(src: Path, dry: bool = False) -> str:
    ...
    if share > MAX_SHARE:
        return f"⚠ {src.name}: снялось бы {share*100:.0f} % — это уже не рисунок, не трогаю"
    if dry:
