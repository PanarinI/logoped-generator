...
def _pick_diverse(cands: Sequence[Dict[str, Any]], n: int, rnd: random.Random,
    ...
    # ⚠ 08-26, поймал автор: «через кубик я вижу одни и те же 8». И видел:
    # ключи сортировки решали ВСЁ, а перемешивание работало лишь при их полном
    # равенстве. У [Р] в начале слова 20 нарисованных слов, но «частотность 3»
    # (десять слов) била «частотность 2» (тринадцать) при любом сиде — восьмёрка
    # выходила одна и та же, кубик только переставлял её по клеткам. То есть
    # кубик обещал другой НАБОР и давал другую РАССАДКУ.
    #
    # Теперь частотность остаётся ярусом, но не приговором: половина клеток
    # берётся из самых знакомых слов, остальное решает сид по всему пулу.
    # Так «рысь», «ручей» и «ракушка» перестают быть невидимыми, а лист не
    # уходит в редкие слова целиком.
    # Ударность и число слогов из ключа СНЯТЫ: сигналы слабые, а сужали они
    # ярус до одного-двух слов и тем гасили кубик второй раз.
    top = [c for c in pool if (c.get("familiarity") or 0) >= 3]
    rest = [c for c in pool if (c.get("familiarity") or 0) < 3]
    rnd.shuffle(top)
    rnd.shuffle(rest)
    half = (n + 1) // 2
    tail = top[half:] + rest
