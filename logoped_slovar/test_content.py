...
def test_scene_sprite_raster_with_vector_fallback():
    ...
    if any(SC.have_sprite(n) for n in ("tree", "bush", "cloud", "mushroom")):
        ...
        # ⚠ 2026-08-24. Здесь стояло «бок о бок с вектором»: ждали, что рядом с
        # растром на сцене останется хотя бы один векторный <path>. Ожидание было
        # верным ровно пока банк был неполон — нарисованы были 3 спрайта из 35.
        # Теперь банк укомплектован, и на «лесу» вектору не осталось места вовсе:
        # каждый предмет сцены пришёл картинкой. Это не поломка, а достижение
        # цели, и ожидание меняется вместе с ней.
        #
        # Запасной путь «нет файла — рисуется как раньше» этой строкой и не
        # проверялся: он проверен выше, ветками «без банка сцена не пустеет» и
        # «без банка растра нет». Здесь проверяем противоположное — что при
        # полном банке лист собран из картинок, а не из смеси.
        check("при полном банке сцена собрана из картинок",
              live.count("<image") >= 3, True)
