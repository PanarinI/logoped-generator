...
def scene_svg(name: str, w: float, h: float,
    ...
    fn(cv)
    # ⚠ 08-26, поймал автор: «не вижу цветные фоны». Цвет БЫЛ, его съедало
    # выцветание. Число 0.42 выбрано под Ч/Б — там сцена это чёрный контур, и
    # бледнить его надо сильно, иначе он спорит с кружками слогов. В цвете
    # предмет и так отступает: он не чёрный, у него своя мягкая заливка, — и
    # та же прозрачность убивает саму краску, ради которой цвет и заказан.
    # Поэтому у цветной сцены свой уровень. Ч/б не тронут: он выверен глазами.
    if colour and opacity <= 0.5:
        opacity = 0.78
