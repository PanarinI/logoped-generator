...
# ЛЕСТНИЦА ТРЁХ СТРОК — по Поповой, в ЕЁ порядке и с ЕЁ содержанием
# (решение автора 08-23, вариант А).
#
# Что было до: волна · зигзаг · та же волна пунктиром. Первая и третья строки
# были ОДНИМ И ТЕМ ЖЕ путём (862 символа, совпадали посимвольно), различие
# только в пунктире, а простейшая линия источника — улитка — была написана
# (`_path_spiral`), но в печать не шла. То есть шапка ссылалась на Попову,
# а бумага показывала нашу склейку.
#
# Что стало, построчно и с тирами:
#   1 ✅ улитка, слитно — «Вариант 1. Самый простой (улитка)… произносить звук
#        не прерываясь»;
#   2 ✅ горки, высота голоса — «Вариант 2. Посложней… Линия идет вверх, и
#        ребенок старается произнести звук более звонко… линия-вниз голос
#        необходимо сделать ниже»;
#   3 ✅ прерывистая, отрывисто — «Вариант 3. Самый сложный. Используются
#        звуковые дорожки с прерывистыми линиями. Ребёнку необходимо коротко
#        или длинно произносить звук».
#
# 🔶 Наше здесь одно: КАКУЮ фигуру рвать пунктиром. Попова говорит «прерывистые
# линии» и формы не называет. Берём горки — тогда третья строка читается как
# «то же, что во второй, но линия рвётся», и лестница нарастает без скачка.
# Задачу голосу третьей строки НЕ удваиваем: в цитате только «коротко или
# длинно», и приписывать ей высоту значило бы утверждать больше, чем можем
# показать (локальный закон 2).
LADDER = (
    (_path_spiral, False, "тяни слитно",
    ...
    (_path_arcs,   False, "голос вверх-вниз",
...
{f'''  <b>Взрослому.</b> Это ступень <b>до слога</b>: ребёнок тянет один звук
  ...
  улитка — тянуть слитно, горки — вести голос выше и ниже, прерывистая —
 ...
 if isolated else '''  <b>Взрослому.</b> Слог на всех трёх дорожках <b>один и тот же</b> — меняется
  <b>задача голосу</b>, и её задаёт форма линии: улитка — тянуть слитно,
  горки — вести голос выше и ниже, прерывистая — произносить отрывисто.
