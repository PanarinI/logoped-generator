...
def block_reason(material: str, sound: str, typ: str) -> str:
    ...
    # звуках, и слоговая дорожка [Ж] печатала «АЖНЬ · ОЖДЬ · ЫЖДЬ» — слогов,
    # которых в русском нет (поймал автор). Причина языковая: стечение ВНУТРИ
    # слова почти всегда открывает следующий слог, а не закрывает текущий
    # («лы-жник», «ко-шка»). Замер по всем картотекам: настоящих закрывающих
    # стечений 31, мнимых 197. Спрашиваем на ПУСТОМ профиле — так отделяем
    # предел языка от нехватки слов у конкретного ребёнка.
    if typ == "cluster_coda" and not C.cluster_frames_for(sound, typ, ()):
        return NO_CODA
    return ""
