...
#
# ⚠ 2026-08-24, решение автора. Классов было ЧЕТЫРЕ — свистящие, шипящие,
# сонорные, заднеязычные. Это канонная четвёрка дислалии, и пока продукт целился
# в дислалию, её хватало. Но наш собственный разбор реальных обращений говорит
# другое (research/logoped_realnye_zaprosy_2026-08-02.md, дословно): «Ни одного
...
#
# Почему это правка ЗДЕСЬ, а не в списке кнопок. CORRIGIBLE строится из этого
# словаря, а `corrigible_of(слово)` возвращает только фонемы из CORRIGIBLE.
# Пока [т] не корригируем, движок его в слове НЕ ВИДИТ: кнопка «Т» в профиле
# была бы нажимаемой и не делала бы ничего — ровно та ложь, которую запрещает
# локальный закон 12. Поймано проверкой: с профилем {к,г,т,д,б,п} на стечения
# по-прежнему выходили «тр», «др», «бр».
#
# Заднеязычные К и Г — тоже смычные, но они стоят в своём классе по канону,
# и мы канонную разметку не трогаем: пятый класс — только губные и переднеязычные.
PHONEME_CLASSES: Dict[str, FrozenSet[str]] = {
    ...
    "смычные":       frozenset({"п", "п'", "б", "б'", "т", "т'", "д", "д'"}),
}
