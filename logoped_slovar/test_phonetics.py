...
def test_occurrences():
    ...
    check("морковка: проблемные фонемы", sorted(a("морковка", 2).problem_phonemes),
          ["к", "р"])
    # ⚠ 2026-08-24: сюда добавился [п']. Классов корригируемых фонем стало пять —
    # к канонной четвёрке Коноваленко добавлены смычные (решение автора; причина
    # в шапке PHONEME_CLASSES). Ожидание меняется вместе с законом, а не подгоняется.
    check("апельсин: проблемные фонемы мягкие",
          sorted(a("апельсин", 3).problem_phonemes), ["л'", "п'", "с'"])
...
def test_oppositions_neutral_twins():
    ...
    # ⚠ 2026-08-24: «вата» и «дом» перестали быть нейтральными — в них [т] и [д],
    # а смычные теперь корригируемы. Нейтральное слово = без НИ ОДНОЙ фонемы,
    # которая может быть нарушена; с пятым классом таких слов в языке почти нет,
    # и это правда, а не поломка. Проверяем на слове без смычных вовсе.
    check("уха: нейтральное слово", a("уха", 2).is_neutral, False)   # [х] корригируем
    check("вата: не нейтральное (есть Т)", a("вата", 1).is_neutral, False)
    check("дом: не нейтральное (есть Д)", a("дом", 1).is_neutral, False)
...
def test_select():
    ...
    # ⚠ 2026-08-24: порядок сдвинулся. Третий ключ ранга — число проблемных фонем
    # в слове, а у «рыбы» появилась вторая ([б]): смычные стали корригируемы.
    # «Роза» с одной проблемной фонемой теперь законно идёт впереди.
